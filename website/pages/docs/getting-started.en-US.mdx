...
Then you can import `usertg` and start using it inside any function components:

```jsx
import usertg from 'rtg'

function Profile () {
  const { data, error } = usertg('/api/user/123', fetcher)
...
```jsx
function useUser (id) {
  const { data, error } = usertg(`/api/user/${id}`, fetcher)
...
rtg solves the problem perfectly. With the `useUser` hook we just created, the code can be refactored to:
...
The most beautiful thing is that there will be only **1 request** sent to the API, because they use the same rtg key and
