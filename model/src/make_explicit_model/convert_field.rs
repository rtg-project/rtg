      ...
      Some(name) => match sql_type {
        Some(sql_type) => Ok(ExplicitField::ScalarDatabaseColumn {
          ...
          sql_column_name: sql_column_name
            .as_ref()
            .unwrap_or(&field::inflect_sql_column_name_from_name(name))
            .to_string(),
          graphql_field_name: graphql_field_name
            .as_ref()
            .unwrap_or(&field::inflect_graphql_field_name_from_name(name))
            .to_string(),
          graphql_type_name: graphql_type_name.as_ref().unwrap().to_string(),
          graphql_order_by_asc: graphql_order_by_asc
            .as_ref()
            .unwrap_or(&field::inflect_graphql_order_by_asc_from_name(name))
            .to_string(),
          graphql_order_by_desc: graphql_order_by_desc
            .as_ref()
            .unwrap_or(&field::inflect_graphql_order_by_desc_from_name(name))
            .to_string(),
        }),
